...
class CardioRiskEngine:
	...
	def evaluate_ecg_variability(self):
		...
		stddev_rr = statistics.stdev(self.ecg_rr_intervals)

		if mean_rr == 0:
			return

		cv_rr = (stddev_rr / mean_rr) * 100
	...
	def apply_signal_penalty(self):
		if self.signal_quality < 0.7:
			self.explanations.append("Signal quality was suboptimal; confidence reduced.")

		if self.signal_quality < 0.5:
			self.explanations.append("Poor signal quality may affect accuracy.")
